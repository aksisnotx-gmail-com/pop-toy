...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="100px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="手办名称" prop="shoubanmingcheng">
            <el-input v-model="ruleForm.shoubanmingcheng" 
                placeholder="手办名称" clearable :disabled=" false  ||ro.shoubanmingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="手办分类" prop="shoubanfenlei">
            <el-select v-model="ruleForm.shoubanfenlei" placeholder="请选择手办分类" :disabled=" false  ||ro.shoubanfenlei" >
              <el-option
  ...
  export default {
    data() {
      ...
      return {
        ...
        ruleForm: {
          ...
          shangpinxiangqing: '',
          tupian: '',
          onelimittimes: Number('2') ,
          alllimittimes: Number('20') ,
          clicktime: '',
          discussnum: '',
          price: '',
          storeupnum: '',
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/shoubanfenlei/shoubanfenlei', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.shoubanfenleiOptions = res.data.data;
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shoubanshangpin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shoubanshangpin/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shoubanshangpin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
